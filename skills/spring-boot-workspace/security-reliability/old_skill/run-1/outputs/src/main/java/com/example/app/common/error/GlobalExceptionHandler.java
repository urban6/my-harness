...
/**
 * 모든 에러 응답을 RFC 9457 ProblemDetail(application/problem+json)로 통일한다.
 * 상태코드 매핑은 이 한 곳에 모은다 — 컨트롤러/서비스에서 분기하지 않는다.
 */
...
public class GlobalExceptionHandler {
    ...
    // 400 — Bean Validation 실패(필수값 누락·이메일 형식 오류 등)
    ...
    public ProblemDetail handleValidation(MethodArgumentNotValidException ex) {
        ...
        pd.setProperty("errors", errors);   // 확장 필드: 필드별 오류
        return pd;
    }
